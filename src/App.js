...
function App() {
  ...
  const [selectedWireLength, setSelectedWireLength] = useState(5); // Add state for the selected wire length
  const [selectedFin, setSelectedFin] = useState('Fin 1'); // Add state for the selected Fin
  const [start, setStart] = useState({ x: -7, y: 5, z: 20 }); // Functional identification number location
  const [target, setTarget] = useState({ x: -5, y: 7, z: 25 }); // Specific location on the surface
  ...
  useEffect(() => {
    ...
    if (path) {
      console.log('Shortest Path:', path); // Log the path
  ...
  const handleFinChange = (e) => {
    ...
    setSelectedFin(fin);
    // Update start based on selected Fin
    switch (fin) {
      case 'Fin 1':
  ...
  console.log('Path Points:', pathPoints); // Log the path points
  ...
  return (
    <>
      ...
      <div>
        <label htmlFor="fin">Functional Identification Number:</label>
      ...
      <div>
        <h5>
          The Green sphere represents the location of the Functional Indentification Number.
        </h5>
        <h5>
        The Red blinking sphere represents the position of the estimated wire fault.
        </h5>
        <h5>
        The Brown line between the green and red Sphere respresents the shortest distance from Fin to wirefault
        </h5>
